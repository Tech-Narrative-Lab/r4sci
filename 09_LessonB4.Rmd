...
Additionally, note the use of `legend()`, `col=`, and other arguments to adjust the behavior of these functions.  The `legend()` function adds a legend to an existing visualization.  `legend` is one of many functions that layer visual elements on top of existing visualizations that are still active in an R graphical device.  Other visualization functions can operate in this mode with the right arguments (typically `add= TRUE`).  
