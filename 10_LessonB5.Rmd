...
LESSON WRITING IN PROGRESS... MORE CONTENT COMING SOON
...
|Loops			|Conditions		|Error Handling		|Expression						|Object Dissection		|Function Dissection
|:-					|:-						|:-								|:-										|:-										|:-
|`repeat{}`	|`if(){}`			|`gc()`								|`cat()`							|`attr()`
|`while(){}`|`else{}`			|`::`									|`capture.output()`		|`attributes()`
|`for(){}`	|`&&`					|`stop()`							|`invisible()`				|`slot()`
|`break`		|`||`					|`unclass()`					|`print()`						|`slotNames()`
|`next`			||`try()`			|`suppressWarnings()`	|`@`
|`optim()`	|							|`warnings()`					|`identity()`					|`str()`
|`Sys.sleep()`|						|`warning()`
...
|`while(){}`		|`try()`				|`identity()`						|`str()`				|`getAnywhere()`
|`optim()`			|`unclass()`		|											|								|`Rprof()`
|`break`				|`warning()`		|											|								|`summaryRprof()`
|`return()`			|								|											|								|`browser()`
|`repeat`			|								|											|								|
|`next`			|								|											|								|